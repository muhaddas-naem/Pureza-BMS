...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addUser = (userData: Omit<User, 'id' | 'createdAt'>) => {
    ...
    logActivity('নতুন ব্যবহারকারী যুক্ত', `'${userData.name}' (${userData.role}) ইউজার অ্যাকসেস যুক্ত করা হয়েছে।`);
  };
  ...
  const updateUser = (updated: User) => {
    ...
    logActivity('ইউজার আপডেট', `'${updated.name}' ইউজারের রোল ও পারমিশন সংশোধন করা হয়েছে।`);
  };
  ...
  const deleteUser = (userId: string) => {
    ...
    if (target) {
      ...
      logActivity('ইউজার ডিলিট', `'${target.name}' ইউজার রিমুভ করা হয়েছে।`);
    }
  };
  ...
  const addOrder = (orderData: Omit<Order, 'id' | 'orderNumber' | 'createdAt' | 'statusHistory'>): Order => {
        ...
        return prevCusts.map((c) =>
          c.phone === orderData.customerPhone
            ? {
                ...
                totalSpent: c.totalSpent + orderData.grandTotal,
    ...
    // Auto Deduct Stock if status is Delivered or Processing
    if (orderData.orderStatus === 'Delivered' || orderData.orderStatus === 'Processing' || orderData.orderStatus === 'Shipped') {
      orderData.items.forEach((item) => {
        adjustStock(item.productId, 'Stock Out', item.quantity, `অর্ডার #${orderNumber} এর জন্য মজুদ হ্রাস`, orderNumber);
      });
    }

    logActivity('নতুন অর্ডার তৈরি', `অর্ডার #${orderNumber} (${orderData.customerName}) সফলভাবে যুক্ত করা হয়েছে।`);
    return newOrder;
  };

  const updateOrder = (updatedOrder: Order) => {
    setOrders((prev) => prev.map((o) => (o.id === updatedOrder.id ? updatedOrder : o)));
    logActivity('অর্ডার তথ্য পরিবর্তন', `অর্ডার #${updatedOrder.orderNumber} এর বিবরণ আপডেট করা হয়েছে।`);
  };
  ...
  const updateOrderStatus = (orderId: string, newStatus: OrderStatus, notes?: string) => {
    ...
    setOrders((prev) =>
      prev.map((o) => {
        if (o.id === orderId) {
          ...
          // Auto stock management on status transitions
          if (
            (newStatus === 'Delivered' || newStatus === 'Processing' || newStatus === 'Shipped') &&
            prevStatus !== 'Delivered' &&
            prevStatus !== 'Processing' &&
            prevStatus !== 'Shipped'
          ) {
            o.items.forEach((item) => {
              adjustStock(item.productId, 'Stock Out', item.quantity, `অর্ডার #${o.orderNumber} স্ট্যাটাস ${newStatus} হওয়ায় স্টক আউট`, o.orderNumber);
            });
          } else if (
            (newStatus === 'Cancelled' || newStatus === 'Returned') &&
            (prevStatus === 'Delivered' || prevStatus === 'Processing' || prevStatus === 'Shipped')
          ) {
            o.items.forEach((item) => {
              adjustStock(item.productId, 'Stock In', item.quantity, `অর্ডার #${o.orderNumber} ${newStatus} হওয়ায় স্টক ফেরত`, o.orderNumber);
            });
          }
    ...
    logActivity('অর্ডার স্ট্যাটাস আপডেট', `অর্ডার #${orderNum} এর স্ট্যাটাস '${newStatus}' করা হয়েছে।`);
  };
  ...
  const addProduct = (prodData: Omit<Product, 'id' | 'createdAt'>) => {
    ...
    logActivity('নতুন পণ্য যোগ', `পণ্য '${prodData.name}' (SKU: ${prodData.sku}) স্টক ইন করা হয়েছে।`);
  };
  ...
  const updateProduct = (updated: Product) => {
    ...
    logActivity('পণ্য আপডেট', `পণ্য '${updated.name}' এর তথ্য পরিবর্তন করা হয়েছে।`);
  };
  ...
  const deleteProduct = (productId: string) => {
    ...
    if (target) {
      ...
      logActivity('পণ্য ডিলিট', `পণ্য '${target.name}' সরানো হয়েছে।`);
    }
  };
  ...
  const addCategory = (name: string, description?: string) => {
    ...
    logActivity('নতুন ক্যাটাগরি', `ক্যাটাগরি '${name}' যুক্ত করা হয়েছে।`);
  };
  ...
  const addExpense = (expData: Omit<Expense, 'id' | 'createdAt'>) => {
    ...
    logActivity('নতুন খরচ', `৳${expData.amount} [${expData.category || expData.categoryName}] যুক্ত করা হয়েছে।`);
  };

  const updateExpense = (updatedExp: Expense) => {
    setExpenses((prev) => prev.map((e) => (e.id === updatedExp.id ? updatedExp : e)));
    logActivity('খরচের তথ্য আপডেট', `'${updatedExp.title}' আপডেট করা হয়েছে।`);
  };
