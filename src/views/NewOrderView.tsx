          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              ...
              <select
                ...
                className="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-teal-500 font-semibold"
              >
                {bdDistricts.map((d) => (
            ...
            {items.map((item, index) => (
              ...
              >
                <div className="sm:col-span-4">
                  <label className="block text-[10px] text-slate-400 mb-0.5">পণ্য সিলেক্ট করুন</label>
                  <select
                  ...
                  >
                    {products.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name} (স্টক: {p.currentStock} টি) - ৳{p.sellingPrice}
                      </option>
                    ))}
                  </select>
                </div>
