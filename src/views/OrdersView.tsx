          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl p-6 space-y-6 border border-slate-200 dark:border-slate-800 my-8">
            ...
            <form onSubmit={handleSaveEditOrder} className="space-y-6 text-xs">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl space-y-3 border border-slate-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    <label className="block font-semibold mb-1">জেলা (District) *</label>
                    <select
                      value={editDistrict}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500">পণ্য যোগ করুন:</span>
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                          handleAddItemFromProducts(e.target.value);
                          e.target.value = '';
                        }
                      }}
                      defaultValue=""
                      className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 font-semibold"
                    >
                      <option value="" disabled>
                        -- তালিকা থেকে সিলেক্ট করুন --
                      </option>
                      {products.map((p) => (
